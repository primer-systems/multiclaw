...
Used by GUI and Console to interact with the MultiClaw core.
"""
...
class CoreClient:
    ...
    Provides a Python API that mirrors the MultiClaw core methods.
    ...
    def start_server(self, port: int = 9402, allow_lan: bool = False) -> dict:
    ...
    # -------------------------------------------------------------------------
    # Events (TODO: WebSocket implementation)
    # -------------------------------------------------------------------------
    ...
    def subscribe(self, event_type: str, handler: Callable[[dict], None]) -> None:
        ...
        For now, GUI should poll or use callbacks.
        """
            ...
            try:
                ...
                try:
                    ...
                    raise CoreClientError(
                        ...
                        error_body.get("code", "HTTP_ERROR"),
                        e.code
                    )
    ...
    def try_connect(
        ...
        """
        Try to connect to a running MultiClaw instance.
        ...
        MultiClaw installations exist on the same machine).
    ...
    def create_policy(self, name: str, daily_limit_micro: Optional[int] = None,
                      ...
                      networks: Optional[list] = None) -> "PolicyProxy":
    ...
    def update_policy(self, policy: "PolicyProxy") -> None:
        """Update a policy."""
        self._put(f"/policies/{policy.id}", {
            "name": policy.name,
    ...
    def server_port(self) -> int:
        ...
        return self.get_status().get("server_port") or 9402

    def start_server(self, port: int = 9402, allow_lan: bool = False) -> bool:
        """Start the agent server."""
        try:
...
class PolicyProxy:
    ...
    @property
    def per_request_max_micro(self) -> int:
        return self._data.get("per_request_max_micro", 0)

    @per_request_max_micro.setter
    def per_request_max_micro(self, value: int):
...
class _SettingsData:
    ...
    class _Server:
        def __init__(self, d: dict):
            self.default_port = d.get("default_port", 9402)
            self.allow_lan = d.get("allow_lan", False)

    class _Display:
        def __init__(self, d: dict):
            self.default_network = d.get("default_network", 8453)

"""
MultiClaw - The central coordinator for all operations.
...
if TYPE_CHECKING:
    from ..models import PolicyStore, Agent, SpendPolicy, Transaction
    from ..wallet import MultiClawWallet
...
class MultiClaw:
    """
    Central coordinator for MultiClaw.
    ...
    def __init__(
        ...
        """
        Initialize MultiClaw.

        Args:
            data_dir: Directory for data files. Defaults to ~/.multiclaw/
        ...
        # Wallet state
        self._unlocked_wallet: Optional["MultiClawWallet"] = None
        ...
        logger.info(f"MultiClaw initialized with data_dir: {self._data_dir}")
    ...
    def get_wallet_for_address(self, address: str) -> Optional["MultiClawWallet"]:
        """
    ...
    def create_wallet(
        ...
        try:
            # Create wallet
            wallet = MultiClawWallet.create(password)
    ...
    def load_wallet(self, wallet_path: str, password: str) -> dict:
        ...
        try:
            wallet = MultiClawWallet.load(wallet_path, password)
            self._unlocked_wallet = wallet
            self._wallet_password = password
    ...
    def get_wallet(self) -> Optional["MultiClawWallet"]:
        """

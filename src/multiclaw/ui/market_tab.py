        ...
        try:
            ...
            while True:
                ...
                req = Request(url, headers={
                    ...
                    "User-Agent": "MultiClaw/1.0 (https://primer.systems)",
                })
...
class MarketTab(QWidget):
    ...
    def _build_ui(self):
        ...
        market_link = QLabel(
            f'<a href="{MARKET_URL}" style="color: {Theme.LIME_DIM};">agentic.market</a>'
        )
    ...
    def _on_service_selected(self, current, previous):
        ...
        # Build endpoint table as HTML
        lines = []
        lines.append(f'<span style="color: #aaaaaa;">{desc}</span>')

        if provider and provider != domain:
            lines.append(f'<span style="color: #888888;">Provider: {provider} · Domain: {domain} · Networks: {networks}</span>')
        else:
            lines.append(f'<span style="color: #888888;">Domain: {domain} · Networks: {networks}</span>')
        ...
        for ep in endpoints:
            ...
            method_color = Theme.LIME if method == "GET" else Theme.WARNING if method == "POST" else Theme.RUST
            lines.append(
                f'<span style="color: {method_color}; font-weight: bold;">{method}</span> '
                f'<span style="color: {Theme.WHITE};">{url}</span>'
            )
            lines.append(
                f'    <span style="color: #999999;">{ep_desc} · {price_str}</span>'
            )
    ...
    def _copy_agent_snippet(self):
        ...
        lines.append("Use the x402 payment protocol. When you receive HTTP 402, "
                      "sign the payment via MultiClaw at localhost:9402/sign and retry with the PAYMENT-SIGNATURE header.")

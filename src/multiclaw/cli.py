#!/usr/bin/env python3
"""
MultiClaw CLI - Command-line interface for MultiClaw.
...
Scripting Mode:
  ...
  Or use MULTICLAW_PASSWORD environment variable for passwords.
"""
...
def parse_global_flags(args: list[str]) -> tuple[list[str], ScriptContext]:
    ...
    """
    auto_confirm = False
    password = os.environ.get("MULTICLAW_PASSWORD")
    remaining = []
...
def single_command_mode(handler: CommandHandler, args: list[str]):
    ...
    Environment variables:
        MULTICLAW_PASSWORD  Password for wallet operations
    """
    # Parse global flags
...
def print_usage():
    ...
    print(f"""MultiClaw CLI v{__version__}

Usage:
  multiclaw                     Start interactive mode (REPL)
  multiclaw <command> [args]    Run a single command
...
Environment Variables:
  MULTICLAW_PASSWORD  Password for wallet operations (alternative to --password)

Examples:
  multiclaw wallet list
  multiclaw wallet open mywallet --password "secret"
  multiclaw policy delete old-policy --yes
  MULTICLAW_PASSWORD="secret" multiclaw wallet create newwallet --yes
...
def main():
    ...
    if core is None:
        ...
        core = MultiClaw(data_dir=data_dir)

...
"""
MultiClaw Signing Helper - Sign x402 payment requests for MultiClaw.
...
def sign_request(agent_id: str, agent_token: str, payment_required: str, request_url: str = None) -> dict:
    ...
    Returns:
        The signed request ready to POST to MultiClaw's /sign endpoint
    """
...
def send_to_multiclaw(signed_request: dict, multiclaw_url: str = "http://localhost:9402") -> dict:
    """
    Send a signed request to MultiClaw and get the payment header.

    Returns the MultiClaw response with payment header on success.
    """
    url = f"{multiclaw_url}/sign"
...
def get_branded_html(port: int) -> str:
    ...
    logo_html = f'<img src="{logo_data_uri}" alt="MultiClaw">' if logo_data_uri else '<span style="color: #baea2a; font-weight: 600; letter-spacing: 2px;">MULTICLAW</span>'
...
<head>
  ...
  <title>MultiClaw - Agent Link</title>
  ...
  <div class="container">
    ...
    <div class="panel">
      ...
      <div class="panel-body">
        <p>This server accepts x402 payment requests from your local AI agents, presents them for user approval in the MultiClaw desktop app, and returns signed authorizations. Your keys never leave the app; agent spending is controlled by your pay policies.</p>
      </div>
    </div>
...
class AgentRequestHandler(BaseHTTPRequestHandler):
    ...
    def _render_receipt_html(self, receipt: dict) -> str:
        ...
        logo_data_uri = get_logo_base64()
        logo_html = f'<img src="{logo_data_uri}" alt="MultiClaw">' if logo_data_uri else '<span style="color: #baea2a;">MULTICLAW</span>'
...
<body>
  <div class="container">
    ...
    <div class="footer">
      Generated by MultiClaw &middot; AP2 Protocol v0.1 &middot; {receipt.get("timestamp", "")}
    </div>
  </div>
</body>
</html>'''

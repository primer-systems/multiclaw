...
Starts the MultiClaw core, admin API, and agent server.
"""
...
class Daemon:
    """
    MultiClaw daemon process.

    Owns:
    - MultiClaw core instance
    ...
    def start(self):
        """Start the daemon."""
        logger.info("Starting MultiClaw daemon...")

        # Initialize core with headless approval handler
        self._core = MultiClaw(data_dir=self._data_dir)
    ...
    def stop(self):
        ...
        logger.info("Stopping MultiClaw daemon...")
...
def run_daemon(
    ...
    Args:
        data_dir: Data directory (defaults to ~/.multiclaw/)
        admin_port: Port for admin API (default 9403)
        agent_port: Port for agent API (default 9402)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="MultiClaw Daemon")
    parser.add_argument(
        "--data-dir",
        type=Path,
        help="Data directory (default: ~/.multiclaw/)"
    )
    parser.add_argument(
        "--admin-port",
        type=int,
        default=9403,
        help="Admin API port (default: 9403)"
    )
    parser.add_argument(
        "--agent-port",
        type=int,
        default=9402,
        help="Agent API port (default: 9402)"
    )
    parser.add_argument(
        "--allow-lan",
        action="store_true",

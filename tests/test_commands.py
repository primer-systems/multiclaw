...
class TestAgentCommands:
    ...
    def test_agent_register(self, handler):
        ...
        assert "MULTICLAW_AGENT_ID" in result.output
...
class TestWalletCommands:
    ...
    def test_wallet_status_no_wallet(self, handler):
        """wallet status when no wallet is loaded."""
        result = handler.execute("wallet status")
        assert result.success
...
class TestSeedCommands:
    ...
    def test_seed_list_wallet_locked(self, handler):
        """seed list when wallet locked."""
        result = handler.execute("seed list")
        assert not result.success
...
class TestAddressCommands:
    ...
    def test_address_list_wallet_locked(self, handler):

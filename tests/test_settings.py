...
class TestAppSettings:
    ...
    def test_default_values(self):
        ...
        assert settings.signing.verify_settlements is False
    ...
    def test_to_dict(self):
        ...
        d = settings.to_dict()
        assert d["version"] == 1
        assert d["signing"]["verify_settlements"] is False
...
class TestSettingsManager:
    ...
    def test_set_network_enabled(self, settings_manager):
        ...
        settings_manager.set_network_enabled(8453, True)
        assert settings_manager.is_network_enabled(8453) is True

        settings_manager.set_network_enabled(8453, False)
        assert settings_manager.is_network_enabled(8453) is False
        settings_manager.stop()
    ...
    def test_set_rpc_endpoint(self, settings_manager):
        ...
        settings_manager.set_rpc_endpoint(8453, "https://custom.rpc.com")
        assert settings_manager.get_rpc_endpoint(8453) == "https://custom.rpc.com"

        # Reset to default
        settings_manager.set_rpc_endpoint(8453, None)
        assert settings_manager.get_rpc_endpoint(8453) is None
        settings_manager.stop()
    ...
    def test_on_change_callback(self, temp_data_dir):
        ...
        manager.set_verify_settlements(True)

        assert len(callback_called) == 1
        assert callback_called[0].signing.verify_settlements is True
        manager.stop()
...
class TestCoreSettingsIntegration:
    ...
    def test_core_set_network_enabled(self, core):
        ...
        core.set_network_enabled(8453, True)
        assert core.is_network_enabled(8453) is True
...
class TestConfigCommands:
    ...
    def test_config_set_network(self, handler):
        ...
        result = handler.execute("config set network 8453 on")
        assert result.success
        assert "enabled" in result.output.lower()

        result = handler.execute("config set network 8453 off")
        assert result.success
        assert "disabled" in result.output.lower()
    ...
    def test_config_set_rpc(self, handler):
        ...
        result = handler.execute("config set rpc 8453 https://custom.rpc.com")
        assert result.success
        assert "8453" in result.output

        result = handler.execute("config set rpc 8453 default")
        assert result.success
...
class TestSettingsEventEmission:
    ...
    def test_settings_changed_event(self, core):
        ...
        assert events_received[0].data["signing"]["verify_settlements"] is True
